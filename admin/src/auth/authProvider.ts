...
export const authProvider: AuthBindings = {
  login: async ({ email, password }) => {
    try {
      ...
      if (!res.ok) {
        ...
        return { success: false, error: new Error(t || 'Login failed') };
      }
      ...
      return { success: false, error: new Error(e?.message || 'Login failed') };
    }
  },
  logout: async () => {
  ...
  onError: async () => ({}),
  check: async () => localStorage.getItem('admin_token')
    ? { authenticated: true }
